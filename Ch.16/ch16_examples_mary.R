...
# set up the data ---------------------------------------------------------
srrs2 <- read.table("srrs2.dat",header=T,sep=",")      # read in the data
...
radon.data <- list(y=y,x=x,n=n,county=county,J=J)      # define data in a list
radon.inits <- list(a=rnorm(J),b=rnorm(1)
                    ,mu.a=rnorm(1), sigma.y=runif(1),  # set up list of initial values to start the sampler
                    sigma.a=runif(1))
...
samps <- coda.samples(jags.mod,params
                      ...
                      ,thin=10)                        # sample every 10 iterations
plot(samps)                                            # trace plots to assess mixing

st <- start(samps)                                     # save for later
en <- end(samps)

# 4. Adapt and burn in
burn.in <- 1000
update(jags.mod, n.iter=burn.in)                       # update to the model to start after burn in
samps <- coda.samples(jags.mod,params,n.iter=2000,thin=10)    # run samples again
