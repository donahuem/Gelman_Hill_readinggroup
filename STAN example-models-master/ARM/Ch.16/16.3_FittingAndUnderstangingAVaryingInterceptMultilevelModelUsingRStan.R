setwd("~/GitHub/Gelman_Hill_readinggroup/STAN example-models-master/ARM/Ch.16")
...
library(shinystan)
radon.1.sso <- launch_shinystan(radon.1.sf)
...
p2 <- ggplot(estimates.ggdf, aes(x = sample.size, y = a.multilevel)) +
    ...
    scale_x_continuous("sample size in county j",
